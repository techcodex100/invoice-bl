...
def extract_invoice_data(text: str) -> dict:
    ...
    # Simple-line fallbacks
    pol_direct = find(r'Port\s*of\s*Loading\s*[:\-]?\s*([^\n]+)')
    pod_direct = find(r'Port\s*of\s*Discharge\s*[:\-]?\s*([^\n]+)')
    ...
    for match in goods_matches:
        ...
        desc_match = re.search(rf'HS\s*CODE\s*:\s*{re.escape(hs)}\s*(.*?)\s*QUANTITY', raw, FLAGS)
        try:
            desc_raw = desc_match.group(1) if desc_match else ""
        except Exception:
            desc_raw = ""
        desc = re.sub(r'\s+', ' ', desc_raw).strip() if desc_raw else ""
        desc_full = desc
    ...
    if not goods:
        ...
        # Description: text between HS CODE line and next TOTAL NET/GROSS/Amount/BIN marker
        desc_after_hs_match = re.search(
            r'HS\s*CODE\s*[:\-]?\s*[0-9\.]{6,10}\s*([\s\S]{0,800}?)(?:TOTAL\s*NET\s*WEIGHT|TOTAL\s*GROSS\s*WEIGHT|Amount\s*Chargeable|BIN\s*NO|DECLARATION|RATE\s*PER|NO\.?\s*OF|$)',
            raw, FLAGS
        )
        desc_after_hs = desc_after_hs_match.group(1) if desc_after_hs_match else ""
        # Also try to include 1-2 lines right before HS CODE (often product names)
        ...
        if total_net_wt or total_gross_wt:
            ...
            desc_full = (desc_block + "\n" + "\n".join(tail)).strip()
        goods.append({
            "hs_code": (hs_only.group(1) if hs_only else find(r'HS\s*CODE\s*:?\s*([0-9\.]+)')),
            "description": re.sub(r'\s+',' ',desc_full),
            "quantity": "",
            "weight": "",
            "packing": "",
            "unit": "",
...
def generate_bl_pdf(data: dict, template_path="image.jpeg") -> bytes:
    ...
    # PORTS
    # PLACE OF ACCEPTANCE: do not print value to keep header clean (per request)
    c.setFont("Helvetica-Bold", 10)
    ...
    c.setFont("Helvetica", 9)
    # Do not render Final Destination value here per request

    # Skip Terms of Payment and Country of Final Destination values in header

    # Pre-Carriage only (do not draw place_of_receipt near the NON-NEGOTIABLE banner)
    c.setFont("Helvetica", 9)
    ...
    for i, good in enumerate(data.get("goods", [])):
        ...
        # Description of Goods – middle column
